...
RSpec.describe Emails::ComplianceStandards, feature_category: :compliance_management do
  include EmailSpec::Matchers

  include_context 'gitlab email notification'

  describe '#compliance_standards_adherence_csv_email', travel_to: '2022-02-24' do
    ...
    let(:filename) { "2022-02-24-33-standards-adherence.csv" }
    let(:content_type) { 'text/csv' }
    let(:csv_data) { 'csv,separated,things' }

    let(:expected_text) do
      'Your standards adherence CSV export for the group "%{group_name}" is attached to this email.'
    end

    let(:expected_html) do
      'Your standards adherence CSV export for the group %{group_link} is attached to this email.'
    end
    ...
    it 'renders an email with attachment' do
      expect(mail.subject).to eq("#{group.name} | #{Date.current.iso8601} Standards adherence export")
