...
RSpec.describe RemoteDevelopment::OrganizationClusterAgentMappingOperations::Delete::Main, feature_category: :workspaces do
  ...
  let(:rop_steps) do
    [
      [RemoteDevelopment::OrganizationClusterAgentMappingOperations::Delete::MappingDeleter, :and_then]
    ]
  end
      ...
      expect do
        ...
        .to invoke_rop_steps(rop_steps)
              ...
              .with_ok_result_for_step(
                {
                  step_class: RemoteDevelopment::OrganizationClusterAgentMappingOperations::Delete::MappingDeleter,
                  returned_message: RemoteDevelopment::Messages::OrganizationClusterAgentMappingDeleteSuccessful.new(
                    ok_message_content
                  )
                }
              )
  ...
  describe "error cases" do
    ...
    where(:case_name, :err_result_for_step, :expected_response) do
      [
        [
          "when MappingDeleter returns OrganizationClusterAgentMappingNotFound",
          {
            step_class: RemoteDevelopment::OrganizationClusterAgentMappingOperations::Delete::MappingDeleter,
            returned_message: lazy { RemoteDevelopment::Messages::OrganizationClusterAgentMappingNotFound.new(err_message_content) }
          },
          {
            status: :error,
            message: lazy { "Organization cluster agent mapping not found: #{error_details}" },
            reason: :bad_request
          },
        ],
        [
