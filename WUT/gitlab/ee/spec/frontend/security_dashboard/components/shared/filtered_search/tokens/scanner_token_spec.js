...
describe('ScannerToken', () => {
  ...
  describe('item selection - toolFilterType: scanner', () => {
    ...
    it('selects "All scanners" when last selected item is deselected', async () => {
    ...
    it('emits an empty filters-changed event when a all scanners is selected', async () => {
      const spy = jest.fn();
      eventHub.$on('filters-changed', spy);
  ...
  describe('QuerystringSync component - toolFilterType: scanners', () => {
    ...
    it('receives `ALL` when "All scanners" option is clicked', async () => {
      await clickDropdownItem('ALL');
