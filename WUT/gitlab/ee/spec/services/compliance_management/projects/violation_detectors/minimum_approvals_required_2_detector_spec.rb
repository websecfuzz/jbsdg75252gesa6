...
RSpec.describe ComplianceManagement::Projects::ViolationDetectors::MinimumApprovalsRequired2Detector, # rubocop:disable RSpec/SpecFilePathFormat -- we keep the digit 2 seperated by underscore to match the source code filename
  ...
  describe '#detect_violations' do
    context 'when there are 2 or more approvers' do
      let(:audit_event) do
        create(:audit_events_project_audit_event,
          project_id: project.id,
          target_type: 'MergeRequest',
          details: { approvers: %w[user1 user2] }
        )
      end
    ...
    context 'when there is fewer than 2 approvers' do
      let(:audit_event) do
        create(:audit_events_project_audit_event,
          target_type: 'MergeRequest',
          project_id: project.id,
          details: { approvers: ['user1'] }
        )
      end
