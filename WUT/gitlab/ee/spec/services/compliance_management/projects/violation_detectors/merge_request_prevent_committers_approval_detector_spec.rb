...
RSpec.describe ComplianceManagement::Projects::ViolationDetectors::MergeRequestPreventCommittersApprovalDetector,
  ...
  describe '#detect_violations' do
    context 'when there are no approving committers' do
      let(:audit_event) do
        create(:audit_events_project_audit_event,
          project_id: project.id,
          target_type: 'MergeRequest',
          details: { approving_committers: [] }
        )
      end
    ...
    context 'when there are approving committers' do
      let(:audit_event) do
        create(:audit_events_project_audit_event,
          target_type: 'MergeRequest',
          project_id: project.id,
          details: { approving_committers: ['committer1'] }
        )
      end
