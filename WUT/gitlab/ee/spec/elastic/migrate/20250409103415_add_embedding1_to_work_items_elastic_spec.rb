...
RSpec.describe AddEmbedding1ToWorkItemsElastic, feature_category: :global_search do
  ...
  describe 'migration', :elastic, :sidekiq_inline do
    ...
    describe '#new_mappings' do
      let(:elastic_knn_field) { { type: 'dense_vector', dims: 768 } }

      it 'returns embedding_1 field with elastic_knn_field configuration' do
        allow(Search::Elastic::Types::WorkItem).to receive(:elastic_knn_field).and_return(elastic_knn_field)

        expect(migration.new_mappings).to eq({ embedding_1: elastic_knn_field })
      end
    end
