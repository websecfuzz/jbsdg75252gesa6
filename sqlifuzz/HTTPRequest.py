...
class HTTPRequest:
    def __init__(self):
        ...
        ## Amsterdam time zone
        #self.created_time = datetime.now(ZoneInfo("Europe/Amsterdam"))
        #self.response_time = None
        self.timing = None
        self.id_ori_request = None
        self.response_code = None
        self.SQL_detected = list()    ## For matched SQL queries
        self.found_values = list()    ## For matched SQL values
        self.error_SQL_detected = None  ## For SQL queries bringing error messages
        self.SQL_injection_detected = False
        self.SQL_injection_proof = None
        self.matched_payload = None
        self.original_path_parts = None

        self.reference_params = None
        self.trigger = None

        self.body_param_dict = None
        self.another_sql_error_detected = False

    def get_matched_paramval_from_found_values(self):
        logger.info(f"[HTTPREQUEST {self.id[-3:]}] Getting matched paramval from found values: {self.found_values}")
        matched_paramvals = list()
        for fv in self.found_values:
            for pv in self.paramvals:
                if str(pv.value) == str(fv):
                    logger.info(f"[HTTPREQUEST {self.id[-3:]}] Found matched paramval: {pv}")
    ...
    def get_null_paramvals(self):
        logger.info(f"[HTTPREQUEST {self.id[-3:]}] Getting null paramvals")
        paramvals = list()
        for pv in self.paramvals:
            if pv.value==None or pv.value=="":
                logger.info(f"[HTTPREQUEST {self.id[-3:]}] Found null paramval: {pv}")
                paramvals.append(pv)
        return paramvals
    ...
    def update_param_from_paramvals(self, add_paramvals=None, is_drop_previous_paramvals=True):
        ...
        logger.info(f"[HTTPREQUEST {self.id[-3:]}] Update url and body param from paramvals:")
        ...
        sorted_paramvals = sorted(paramvals, key=lambda p: p.param or "") ## Handle None param case
        for paramval in sorted_paramvals:
            if paramval.position==ParamValuePosition.URL:
                ...
                try:
                    if paramval.param and paramval.param.startswith("path"):
                        index = int(paramval.param[4:])  # Get number after "path"
                        # Replace the corresponding part in the original path
                        if 0 <= index < len(self.original_path_parts):
                            if self.original_path_parts[index] != paramval.value:
                                self.original_path_parts[index] = paramval.value                            
                                is_path_changed = True
                except (ValueError, IndexError):
                    logger.info(f"[HTTPREQUEST {self.role}] Error updating path paramval: {paramval} --> {traceback.format_exc()}")

            elif paramval.position==ParamValuePosition.HEADER:
                self.header[paramval.param] = paramval.value
        ...
        if is_path_changed:
            if self.original_path_parts:
                new_paths = "/" + "/".join(self.original_path_parts)
                self.url = self.base_url+new_paths
                logger.info(f"[HTTPREQUEST {self.id[-3:]}] New path: {new_paths}")
    ...
    def extract_param_value(self):
        ...
        if self.header:
            for key in self.header:
                if key.lower() not in STANDARD_HEADERS:
                    pv = ParamValue(key,self.header[key],ParamValuePosition.HEADER)
                    pv.role = self.role
                    self.paramvals.append(pv)
                    dictionary.add(pv)
                    if not pv.is_nonce:
        ...
        # if path_parts:
        if self.original_path_parts:
            for i, p in enumerate(self.original_path_parts):
                ## Only extract the last part of the path as param value
                if i==len(self.original_path_parts)-1:
...
def construct_path(request: HTTPRequest):
    """
    Reconstruct the full path from the request's original URL and modified PATH paramvals.
    If a PATH paramval is modified, it replaces the corresponding part in the original path.
    Returns the complete URL with the modified path component(s).
    """
    # Start with the original parsed URL
    if not request.url:
        return request.base_url
    
    parsed_url = urlparse(request.url)
    original_path_parts = [part for part in parsed_url.path.split("/") if part]
    
    # Apply modifications from PATH paramvals
    for pv in request.paramvals:
        if pv.position == ParamValuePosition.PATH:
            # Extract the index from param name (e.g., "path4" -> 4)
            try:
                if pv.param and pv.param.startswith("path"):
                    index = int(pv.param[4:])  # Get number after "path"
                    # Replace the corresponding part in the original path
                    if 0 <= index < len(original_path_parts):
                        original_path_parts[index] = pv.value
            except (ValueError, IndexError):
                pass
    
    # Reconstruct the full URL with modified path
    if original_path_parts:
        new_path = "/" + "/".join(original_path_parts)
    else:
        new_path = "/"
    
    return request.base_url + new_path
...
def parse_multipart_content(data, content_type):
    NAME_KEYWORD = "name="
    logger.info(f"[HTTPREQUEST] Parsing Multipart Content with content-type: %s", content_type)
    post_data = {}
    ...
    try:
        ...
        for part in multipart_data.parts:
            ...
            try:
                ...
                logger.info(f"[HTTPREQUEST] Error in decoding byte values. Try to use the raw byte")
                logger.info(e)
                post_data[key] = part.content

    except Exception as e:
        logger.info(f"[HTTPREQUEST] Error in parsing multipart content: {e}")
        logger.info(traceback.format_exc()[:1500])
...
def convert_request_from_entry(request_entry) -> HTTPRequest:
    logger.info(f"[HTTPREQUEST {request_entry['sid']}] Converting Request Entry to HTTP Request: %s", request_entry["id"])
    req = None
    try:
        req = HTTPRequest()
        req.source = "InterceptedRequest"
        ...
        # if 'content-type' in request_entry['request']:
        ...
        #        req.timing = request.timing

        # Split the path by "/" and filter out empty strings
        path_parts = [part for part in parsed_url.path.split("/") if part]
        request_entry['path_parts'] = path_parts
        req.original_path_parts = path_parts

        if req.content_type and req.content_type.find('multipart/form-data')>-1:
            logger.info(f"[HTTPREQUEST {request_entry['id'][-3:]}] Seeing a post data with multipart/form-data content-type in %s", request_entry["id"])
            req.is_HTML_form = True
            ...
            logger.info(f"[HTTPREQUEST {request_entry['id'][-3:]}] DATA JSON: %s",data_json)
            req.post_data_json = data_json
        elif req.content_type and req.content_type.find('x-www-form-urlencoded')>-1:
            logger.info(f"[HTTPREQUEST {request_entry['id'][-3:]}] Seeing a post data with x-www-form-urlencoded content-type in %s", request_entry["id"])
            req.is_HTML_form = True
            req.post_data_json = parse_post_data(request_entry['request']['body'])
            req.post_data_encoded = request_entry['request']['body']
        else:
            logger.info(f"[HTTPREQUEST {request_entry['id'][-3:]}] No content-type is found")
        ...
        logger.error(f"[HTTPREQUEST {request_entry['id'][-3:]}] Error in printing result: {e}")
        logger.error(f"[HTTPREQUEST {request_entry['id'][-3:]}] {traceback.format_exc()[-1500:]}")
